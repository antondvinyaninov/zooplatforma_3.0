...
export default function PetGeneralInfo({
  ...
  React.useEffect(() => {
    fetch('/api/pethelper/breeds').then(r => r.json()).then(d => {
  ...
  const renderEditableRow = ({
    ...
    }} // optional: auto-cancel on leave if not interacted
    >
        <div style={{
        fontSize: 13,
        color: '#6b7280',
        ...
        gap: 8
            ...
            {as === 'select' ? <select value={editValue} onChange={e => setEditValue(e.target.value)} style={{
          flex: 1,
          minWidth: 0,
          padding: '4px 8px',
          borderRadius: 6,
          ...
          fontSize: 14
              ...
              </select> : as === 'date' ? <input type="date" value={editValue} onChange={e => setEditValue(e.target.value)} style={{
          flex: 1,
          minWidth: 0,
          padding: '4px 8px',
          borderRadius: 6,
          border: '1px solid #d1d5db',
          fontSize: 14
        }} autoFocus /> : as === 'textarea' ? <textarea value={editValue} onChange={e => setEditValue(e.target.value)} style={{
          flex: 1,
          minWidth: 0,
          padding: '8px',
          borderRadius: 6,
          ...
          minHeight: '80px',
          resize: 'vertical'
        }} autoFocus placeholder={placeholder} onKeyDown={e => {
          if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) saveEdit(field, keyToSave);
          if (e.key === 'Escape') cancelEdit();
        }} /> : <input type="text" value={editValue} onChange={e => setEditValue(e.target.value)} style={{
          flex: 1,
          minWidth: 0,
          padding: '4px 8px',
          borderRadius: 6,
          ...
          if (e.key === 'Escape') cancelEdit();
            ...
            <button onClick={() => saveEdit(field, keyToSave)} disabled={saving} style={{
          width: 28,
          height: 28,
          borderRadius: 6,
          border: 'none',
        ...
        minHeight: 28
            ...
            <button onClick={() => startEdit(field, value)} title="Редактировать" style={{
          width: 28,
          height: 28,
          borderRadius: 6,
          border: 'none',
  ...
  const renderEditableBreedRow = () => {
    ...
    const filteredBreeds = breeds.filter(b => (!pet.species_id || b.species_id === pet.species_id) && b.name.toLowerCase().includes(search.toLowerCase()));
    ...
    return <div style={{
      ...
      position: 'relative'
        ...
        position: 'relative'
          ...
          minWidth: 0
          ...
          }} onFocus={() => setShowDropdown(true)} placeholder="Начните вводить..." style={{
            width: '100%',
            minWidth: 0,
            padding: '4px 8px',
            borderRadius: 6,
            ...
            boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)'
                  ...
                  {filteredBreeds.length > 0 ? filteredBreeds.map(b => <div key={b.id} onClick={() => onSelectBreed(b.id, b.name)} style={{
              padding: '8px 12px',
              cursor: 'pointer',
              borderBottom: '1px solid #f3f4f6',
              fontSize: 14
    ...
    return <div style={{
      ...
      border: '1px solid #e5e7eb'
          ...
          flex: 1
            ...
            flex: 1
              ...
              marginBottom: 4
                ...
                <input type="number" min="0" max="30" value={years} onChange={e => setYears(Number(e.target.value))} style={{
              width: '100%',
              padding: '6px 10px',
              borderRadius: 6,
              border: '1px solid #d1d5db'
            ...
            flex: 1
              ...
              marginBottom: 4
                ...
                <input type="number" min="0" max="11" value={months} onChange={e => setMonths(Number(e.target.value))} style={{
              width: '100%',
              padding: '6px 10px',
              borderRadius: 6,
              border: '1px solid #d1d5db'
        ...
        gap: 8
          ...
          <button onClick={saveAge} disabled={saving} style={{
          flex: 1,
          padding: 8,
          borderRadius: 6,
          border: 'none',
    ...
    gap: 24
      ...
      <div>
        ...
        <div className={s.card}>
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-5 xl:gap-6">
            ...
            {/* Блок возраста */}
            {isEditingAge ? <AgeEditor /> : <div style={{
            gridColumn: '1 / -1',
            borderBottom: '1px solid #f3f4f6',
            paddingBottom: 12
          }}>
              ...
              gap: 8
                  ...
                  <button onClick={startEditAge} title="Редактировать возраст" style={{
                width: 28,
                height: 28,
                borderRadius: 6,
                border: 'none',
                ...
                flex: 1
                  ...
                  minWidth: 140
                    ...
                    alignItems: 'center'
                        ...
                        {pet.age_type === 'approximate' && <span style={{
                      color: '#9ca3af',
                      fontSize: 13,
                      marginLeft: 6
                    }}>(примерно)</span>}
      ...
      <div>
        ...
        <div className={s.card}>
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-5 xl:gap-6">
            ...
            label: "Окрас",
            value: pet.color || '',
            displayValue: pet.color || '—'
          })}
            {renderEditableRow({
            field: "fur",
            label: "Шерсть",
            value: pet.fur || '',
            displayValue: pet.fur || '—'
          })}
            {renderEditableRow({
            field: "ears",
            label: "Уши",
            value: pet.ears || '',
            displayValue: pet.ears || '—'
          })}
            {renderEditableRow({
            field: "tail",
            label: "Хвост",
            value: pet.tail || '',
            displayValue: pet.tail || '—'
          })}
            {renderEditableRow({
            field: "special_marks",
            label: "Особые приметы",
            value: pet.special_marks || '',
            displayValue: pet.special_marks || '—'
          })}
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className={s.card}>
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-5 xl:gap-6">
            {renderEditableRow({
            field: "sterilization_date",
            label: "Дата стерилизации (кастрации)",
            ...
            {renderEditableRow({
            field: "sterilization_specialist",
            label: "Специалист",
            ...
            {renderEditableRow({
            field: "sterilization_org",
            label: "Организация (клиника)",
