...
export default function PetIdentification({
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        {pet.user_id && pet.relationship === 'owner' ? (
          ...
          <>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-6 border border-blue-200">
              ...
              <div className="mt-4 pt-4 border-t border-blue-200 flex gap-3">
                <a
                  href={`/users/${pet.user_id || pet.owner_id}`}
                  target="_blank"
          ...
          <div className="space-y-4">
            ...
            {/* Куратор (показываем только если нет владельца и куратор есть) */}
            {pet.user_id && pet.relationship === 'curator' && (
              <div>
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-4">
                      ...
                      {pet.user?.avatar ? (
                        <img
                          src={pet.user.avatar}
                          alt={pet.owner_name || pet.owner_email || 'Куратор'}
                          className="w-16 h-16 rounded-full object-cover shadow-lg border-2 border-white"
                        />
                      ) : (
                      ...
                      {/* Информация */}
                      <div>
                        <h4 className="text-xl font-bold text-gray-900 mb-1">{pet.owner_name || pet.owner_email || pet.user?.name || pet.user?.email || 'Не указан'}</h4>
                        <p className="text-sm text-gray-600">
                          ID пользователя:{' '}
                          <span className="font-mono font-semibold">#{pet.user_id}</span>
                        </p>
                      </div>
                    </div>
                  </div>
