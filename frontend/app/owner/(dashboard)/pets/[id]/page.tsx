...
export default function OwnerPetCardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const orgId = 'owner'; // Фиксируем orgId для Owner, общие компоненты поймут, что это Owner
  ...
  useEffect(() => {
    fetch(`/api/owner/pets/${petId}`, { credentials: 'include' })
      .then((r) => r.json())
      .then((data) => {
        if (data.success || data.data || data.pet) {
          ...
          setItems([
            { label: 'Мои питомцы', href: `/owner/pets` },
  ...
  const handleQuickUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await fetch(`/api/owner/pets/${pet.id}`, {
