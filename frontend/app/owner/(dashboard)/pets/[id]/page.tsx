  ...
  const fetchPet = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // API возвращает {success: true, data: {...}} или {success: true, pet: {...}}
        const petData = data.pet || data.data;

        if (!petData) {
          console.error('No pet data in response:', data);
        ...
        setEditData({
          ...
          relationship: (petData.relationship as 'owner' | 'curator') || 'owner',
          color: petData.color || '',
          fur: petData.fur || '',
          ears: petData.ears || '',
          tail: petData.tail || '',
        ...
        setBirthDateType((petData.age_type as 'exact' | 'approximate') || 'exact');
      } else {
