...
async function proxyRequest(request: NextRequest, params: { path: string[] }) {
  try {
    ...
    const backendUrl = `${BACKEND_API_URL}/api/org/${path}${queryString}`;

    console.log(`[Org Proxy] ${request.method} ${backendUrl}`);

    const headers = new Headers();
    ['authorization', 'content-type', 'user-agent', 'accept', 'accept-language'].forEach((h) => {
    ...
    response.headers.forEach((value, key) => {
      if (!headersToSkip.includes(key.toLowerCase())) responseHeaders.set(key, value);
    });
