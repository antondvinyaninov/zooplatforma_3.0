...
export default function PetCardPage({ params }: { params: Promise<{ orgId: string; petId: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/org/${orgId}/pets/${petId}`, { credentials: 'include' })
      .then((r) => r.json())
      .then((data) => {
        if (data.success) {
          setPet(data.pet);
          setPhotoUrl(data.pet.photo_url || data.pet.media_urls?.[0] || '');
          setItems([
            { label: 'Питомцы', href: `/org/${orgId}/pets` },
            { label: `Питомец №${data.pet.org_pet_number}` },
          ]);
        } else {
  ...
  const handleQuickUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Если главного фото ещё нет — ставим это
      if (!photoUrl) body.photo_url = uploaded.url;
      await fetch(`/api/org/${orgId}/pets/${pet.id}`, {
  ...
  const isdog = pet.species_name === 'Собака';
    ...
    switch (activeTab) {
      ...
      case 'fundraising':    return (
        <div style={{ background: '#fff', borderRadius: 16, padding: 40, textAlign: 'center', boxShadow: '0 1px 12px rgba(0,0,0,0.08)' }}>
          ...
          <p style={{ color: '#6b7280', fontSize: 14 }}>Данный функционал пока находится в разработке и скоро будет доступен. Спасибо за ожидание!</p>
        </div>
      );
    }
  };
    ...
    <div style={{ paddingLeft: 16, paddingRight: 16, paddingBottom: 32 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr 280px', gap: 16, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {/* Навигационное меню */}
          <PetNavMenu activeTab={activeTab} onChange={setActiveTab} showFundraising={pet.catalog_status === 'needs_help'} />
        </div>
          ...
          <div style={{ position: 'sticky', top: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                    ...
                    <label style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', cursor: 'pointer' }}>
                      <input 
                        ...
                        onChange={async (e) => {
                          const newStatus = e.target.checked ? 'looking_for_home' : 'draft';
                          setPet({...pet, catalog_status: newStatus});
                          if (!e.target.checked && activeTab === 'fundraising') {
                            setActiveTab('timeline');
                          }
                          await fetch(`/api/org/${orgId}/pets/${petId}`, {
                ...
                <button 
                  onClick={async () => {
                    if (confirm('Вы уверены, что хотите удалить карточку питомца? Это действие необратимо.')) {
                      try {
                        const res = await fetch(`/api/org/${orgId}/pets/${petId}`, { method: 'DELETE' });
                        if (res.ok) {
                          router.push(`/org/${orgId}/pets`);
                        } else {
            ...
            <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 1px 12px rgba(0,0,0,0.08)', padding: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <div>
                  <div style={{ fontSize: 11, color: '#9ca3af', marginBottom: 1 }}>№ в организации</div>
                  <div style={{ fontSize: 13, color: '#374151', fontWeight: 600 }}>№{pet.org_pet_number}</div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 13, color: '#374151', fontWeight: 600 }}>
                    {pet.relationship === 'owner' ? 'Владелец' : 
                     pet.relationship === 'curator' ? 'Куратор' : 
                     pet.relationship === 'guardian' ? 'Опекун' : 
                     (
                       <span>
                         Опекун (<a href={`/orgs/${orgId}`} target="_blank" rel="noopener noreferrer" style={{ color: '#2563eb', textDecoration: 'none' }} onMouseEnter={e => e.currentTarget.style.textDecoration='underline'} onMouseLeave={e => e.currentTarget.style.textDecoration='none'}>{pet.org_name || 'Организация'}</a>)
                       </span>
                     )}
                  </div>
                </div>
