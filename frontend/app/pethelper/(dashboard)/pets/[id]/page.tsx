...
export default function PethelperPetCardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const orgId = 'pethelper'; // Фиксируем orgId для Pethelper, общие компоненты поймут, что это Pethelper
  ...
  useEffect(() => {
    fetch(`/api/pethelper/pets/${petId}`, { credentials: 'include' })
      .then((r) => r.json())
      .then((data) => {
        if (data.success || data.data || data.pet) {
          ...
          setItems([
            { label: 'Мои питомцы', href: `/pethelper/pets` },
  ...
  const handleQuickUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await fetch(`/api/pethelper/pets/${pet.id}`, {
                    ...
                    <label style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', cursor: 'pointer' }}>
                      <input 
                        ...
                        onChange={async (e) => {
                          ...
                          await fetch(`/api/pethelper/pets/${petId}`, {
                ...
                <button 
                  onClick={async () => {
                    if (confirm('Вы уверены, что хотите удалить карточку питомца? Это действие необратимо.')) {
                      try {
                        const res = await fetch(`/api/pethelper/pets/${petId}`, { method: 'DELETE' });
                        if (res.ok) {
